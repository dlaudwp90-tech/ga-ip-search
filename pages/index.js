...
// ─── 상태 옵션 (추후 항목 추가: 이 배열에 객체 하나만 추가하면 자동 반영) ──
const STATUS_OPTIONS = [
  ...
  // 추가 예시: { key: "hold", label: "보류", color: "#7c3aed", bg: "#f5f3ff", darkColor: "#c4b5fd", darkBg: "#3b0764" },
];
...
export default function Home() {
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // 검토 열 헤더 (inline — styled-jsx 스코핑 적용을 위해 외부 컴포넌트 사용 안 함)
  // ─────────────────────────────────────────────────────────────────────────
  const reviewTh = (
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // 검토 pill 셀 렌더 함수 (inline — 완전 inline 스타일 사용)
  // ─────────────────────────────────────────────────────────────────────────
  const reviewTd = (row, i) => {
    ...
    return (
      ...
      >
        <div style={{ display:"flex", flexDirection:"column", gap:3, opacity: saving ? 0.6 : 1 }}>
  ...
  return (
    <>
                  ...
                  <table>
                    ...
                    <tbody>
                      {results.map((row, i) => (
                        ...
                        >
                          {/* ── 검토 pill 셀 (inline, 완전 inline 스타일) ── */}
                          {reviewTd(row, i)}
      ...
      <style jsx>{`
        ...
        /* ── 검토 열 th — left:0 sticky (inline 렌더라 CSS 정상 적용) ── */
        .th-check { position:sticky; left:0; top:0; z-index:6;
          width:130px; min-width:130px; padding:8px 10px;
          background:#1a3a8f; border-right:2px solid rgba(255,255,255,0.4) !important;
          display:flex; flex-direction:column; align-items:center; gap:3px; vertical-align:middle; }
