        ...
        <tbody>
          {rows.map((row, i) => (
            <tr key={i} className={`result-row ${i % 2 === 0 ? "row-even" : "row-odd"}`}>
              ...
              <td className="td-center">
                <ColorBadge
                  text={renderSingleLine(row.type)}
              ...
              <td className="td-center">
                <ColorBadge
                  text={renderSingleLine(row.status)}
              ...
              <td className="td-center">
                <ColorBadge
                  text={renderSingleLine(row.category)}
              ...
              {/* 출원번호 - 너비 제한 없음 */}
              <td className={`td-appnum ${isMultiLine(row.appNum) ? "td-top" : "td-center"}`}>
  ...
  return (
    <>
      ...
      <style jsx>{`
        ...
        .dark .table-outer { border-color: #334155; background: #1e293b; box-shadow: 0 4px 32px rgba(0,0,0,0.3); }

        table {
          border-collapse: separate;
          border-spacing: 0;
          font-size: 13px;
          width: max-content;        /* 내용 길이에 따라 자동 확장 */
          min-width: 100%;
        }
        ...
        .dark td { border-bottom-color: #2d3f5e; border-right-color: #2d3f5e; }

        /* 출원번호 - 너비 제한 없음 */
        .td-appnum { min-width: 0; width: auto; }
        ...
        /* 제목 셀 - 좌측 정렬 */
        .cell-inner { display: flex; align-items: center; gap: 7px; justify-content: flex-start; }
        ...
        .cell-empty { color: #d1d5db; font-size: 13px; }
