...
export default function Home() {
  ...
  const handleClear = () => {
    ...
    setTableVisible(false); setIsRecent(false);
    fetchRecent();
  ...
  return (
    <>
      ...
      {filePopup && results && (() => {
        ...
        return (
          <div ref={filePopupRef}
            style={{ position:"fixed", left:popupPos.x+14, top:popupPos.y-10, zIndex:500, background:"#fff", border:"1.5px solid #e5e9f5", borderRadius:10, boxShadow:"0 8px 24px rgba(19,39,79,0.18)", padding:6, minWidth:140, display:"flex", flexDirection:"column", gap:4 }}
            onMouseDown={(e) => e.stopPropagation()}
            onTouchStart={(e) => e.stopPropagation()}>
            <a href={link} target="_blank" rel="noreferrer"
              style={{ fontSize:12, fontWeight:700, padding:"8px 14px", borderRadius:7, textDecoration:"none", textAlign:"center", background:"#eef1fb", color:"#1a3a8f", display:"block" }}
              onMouseDown={(e) => e.stopPropagation()}
              onClick={() => setFilePopup(null)}>🔍 미리보기</a>
            <button
              style={{ fontSize:12, fontWeight:700, padding:"8px 14px", borderRadius:7, textAlign:"center", background:downloading[dlKey]?"#f3f4f6":"#f0fdf4", color:downloading[dlKey]?"#9ca3af":"#166534", border:"none", cursor:downloading[dlKey]?"not-allowed":"pointer", fontFamily:"inherit" }}
              onMouseDown={(e) => e.stopPropagation()}
              onTouchStart={(e) => e.stopPropagation()}
            ...
            results.length === 0 ? (
              ...
              <div className={`fade-wrap${tableVisible ? " visible" : ""}`}>
                <div className="count-row">
                  ...
                  {isRecent && (
                    <button className="guide-btn" onClick={() => router.push("/guide")}>
                      📋 문서 작성 방법 및 양식
                    </button>
                  )}
                </div>
                    ...
                    <tbody>
                      {results.map((row, i) => (
                        <tr key={i} className={`result-row ${i % 2 === 0 ? "row-even" : "row-odd"}`}>
                          ...
                          {/* 유형 */}
                          <td className="td-nowrap">
                            {row.typeItems && row.typeItems.length > 0 ? (
                          ...
                          <td className="td-files">
                            {row.fileLinks ? (() => {
                              ...
                              const showFiles = isExpanded ? files : files.slice(0, COLLAPSE_LIMIT);
                              const hasMore = files.length > COLLAPSE_LIMIT;
                              const hiddenCount = files.length - COLLAPSE_LIMIT;
                              return (
                                <div className="file-expand-wrap">
                                  <div className="file-links">
                                    {showFiles.map((link, j) => {
                                      const fileName = decodeURIComponent(link.split("/").pop());
                                      const popupKey = `${i}-${j}`;
                                        ...
                                        <div key={j} className="file-link-wrap">
                                          <span
                                            ...
                                            onMouseDown={(e) => {
                                              ...
                                              else { setPopupPos({ x: e.clientX, y: e.clientY }); setFilePopup(popupKey); }
                                            }}
                                          >
                                            📄 {fileName} ▾
                                          </span>
                          ...
                          {/* 카테고리 */}
                          <td className="td-nowrap">
                            {row.categoryItems && row.categoryItems.length > 0 ? (
                              <div style={{display:"flex",flexWrap:"wrap",gap:3,justifyContent:"center"}}>
      ...
      <style jsx>{`
        ...
        .logo-hint { font-size: 11px; color: #94a3b8; margin-top: 6px; opacity: 0; transition: opacity 0.2s; }
        ...
        .guide-btn { background: #13274F; color: #fff; border: none; border-radius: 8px; padding: 7px 14px; font-size: 12px; font-weight: 700; cursor: pointer; font-family: inherit; white-space: nowrap; transition: background 0.15s; flex-shrink: 0; }
        .guide-btn:hover { background: #0d1e3d; }
        .dark .guide-btn { background: #1e3a6e; }
        .dark .guide-btn:hover { background: #2a4a8e; }
        ...
        .expand-btn.expanded:hover { background: #bae6fd; }
        .dark .expand-btn { background: #1e3a6e; color: #93c5fd; }
        .dark .expand-btn:hover { background: #2a4a8e; }
        .dark .expand-btn.expanded { background: #0c2a3e; color: #7dd3fc; }
        .dark .expand-btn.expanded:hover { background: #0e3a54; }
