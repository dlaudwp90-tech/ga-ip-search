  ...
  return (
    <>
                          ...
                          <td>
                            <div className="cell-inner">
                              ...
                              <span className="doc-title" onClick={(e) => handleTitleClick(e, row.url)}>
                                {renderWithLineBreaks(row.title)}
                              </span>
                            </div>
                          </td>
                          <td>{row.type ? <span className="badge type">{renderWithLineBreaks(row.type)}</span> : <span className="dash">—</span>}</td>
                          <td>{row.status ? <span className="badge status">{renderWithLineBreaks(row.status)}</span> : <span className="dash">—</span>}</td>
                          <td>{row.category ? <span className="badge category">{renderWithLineBreaks(row.category)}</span> : <span className="dash">—</span>}</td>
                          <td>
      ...
      <style jsx>{`
        ...
        .cell-inner { display: flex; align-items: flex-start; gap: 6px; }
        .doc-icon { font-size: 14px; flex-shrink: 0; margin-top: 2px; }
        .doc-title {
        ...
        .badge { border-radius: 5px; padding: 2px 7px; font-size: 11px; font-weight: 700; display: inline-block; line-height: 1.6; }
