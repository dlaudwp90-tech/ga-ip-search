...
function renderWithIndent(text) {
  ...
  return (
    <div className="indent-block">
      {lines.map((line, i) => (
        <div key={i} className={i > 0 ? "indent-line" : "first-line"}>
          {line || "　"}
        </div>
      ))}
    </div>
  ...
  return (
    <>
                    ...
                    <tbody>
                      {results.map((row, i) => (
                        <tr key={i} className="result-row">
                          ...
                          <td className="td-nowrap">
                            {row.status
                              ? <span className="badge status">{renderSingleLine(row.status)}</span>
                              : <span className="dash">—</span>}
                          </td>
                          {/* 카테고리 - 1행 고정 */}
                          <td className="td-nowrap">
                            {row.category
                              ? <span className="badge category">{renderSingleLine(row.category)}</span>
                              : <span className="dash">—</span>}
                          </td>
                          {/* 출원번호 - 들여쓰기 + 복사 */}
      ...
      <style jsx>{`
        ...
        /* 들여쓰기 허용 td */
        .td-indent { vertical-align: top; padding-top: 10px; }

        /* 제목 셀 */
        .cell-inner { display: flex; align-items: center; gap: 6px; white-space: nowrap; }
        ...
        /* 들여쓰기 블록 */
        .indent-block { display: flex; flex-direction: column; gap: 2px; }
        .first-line { font-size: 12px; color: #6b7280; white-space: nowrap; }
        .indent-line { font-size: 12px; color: #6b7280; white-space: nowrap; padding-left: 14px; }
